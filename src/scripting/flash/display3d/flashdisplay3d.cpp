	...
	switch (action.action)
	{
		...
		case RENDER_RENDERTOBACKBUFFER:
			if (renderingToTexture)
			{
				...
				engineData->exec_glBindTexture_GL_TEXTURE_2D(backframebufferID[currentactionvector]);
		...
		case RENDER_TOTEXTURE:
		{
			...
			{
				if (tex->depthRenderBuffer == UINT32_MAX)
				{
					tex->depthRenderBuffer = engineData->exec_glGenRenderbuffer();

					if (engineData->supportPackedDepthStencil)
					{
						engineData->exec_glBindRenderbuffer(tex->depthRenderBuffer);
						engineData->exec_glRenderbufferStorage_GL_RENDERBUFFER_GL_DEPTH_STENCIL(action.udata2,action.udata3);
						engineData->exec_glFramebufferRenderbuffer_GL_FRAMEBUFFER_GL_DEPTH_STENCIL_ATTACHMENT(tex->depthRenderBuffer);
					}
					else
					{
						if (tex->stencilRenderBuffer == UINT32_MAX)
							tex->stencilRenderBuffer = engineData->exec_glGenRenderbuffer();
						engineData->exec_glBindRenderbuffer(tex->depthRenderBuffer);
						engineData->exec_glRenderbufferStorage_GL_RENDERBUFFER_GL_DEPTH_COMPONENT16(action.udata2,action.udata3);
						engineData->exec_glBindRenderbuffer(tex->stencilRenderBuffer);
						engineData->exec_glRenderbufferStorage_GL_RENDERBUFFER_GL_STENCIL_INDEX8(action.udata2,action.udata3);
						engineData->exec_glFramebufferRenderbuffer_GL_FRAMEBUFFER_GL_DEPTH_ATTACHMENT(tex->depthRenderBuffer);
						engineData->exec_glFramebufferRenderbuffer_GL_FRAMEBUFFER_GL_STENCIL_ATTACHMENT(tex->stencilRenderBuffer);
					}
		...
		case RENDER_DRAWTRIANGLES:
		{
			...
			if (currentframebuffertextureid != UINT32_MAX
				&& !currentframebufferdirty
				&& enableDepthAndStencilTextureBuffer)
			{
				// specs are not clear but it seems that we have to clear the depth/stencil buffer
				// on first drawing when rendering to texture
			...
			if (bufferID != UINT32_MAX)
			{
				...
				LOG(LOG_ERROR,"Context3D: missing index bufferID in DrawTriangles, should not happen");
		...
		case RENDER_UPLOADINDEXBUFFER:
		{
			...
			if (bufferID != UINT32_MAX)
			{
				...
				LOG(LOG_ERROR,"Context3D: missing index bufferID for uploading, should not happen");
			break;
		}
		...
		case RENDER_SETTEXTUREAT:
		{
			...
			if (action.udata3)
			{
				...
				engineData->exec_glBindTexture_GL_TEXTURE_2D(0);
				samplers[action.udata1] = UINT32_MAX;
			}
			else
			{
				//LOG(LOG_INFO,"RENDER_SETTEXTUREAT:"<<action.udata2<<" "<<action.udata1<<" "<<currentprogram->gpu_program);
				engineData->exec_glActiveTexture_GL_TEXTURE0(action.udata1);
				engineData->exec_glBindTexture_GL_TEXTURE_2D(action.udata2);
		...
		case RENDER_SETSAMPLERSTATE:
			...
			if (currentprogram
				&& action.udata1 < currentprogram->samplerState.size()
				&& (currentprogram->samplerState[action.udata1].special & SAMPLERSPECIAL_IGNORESAMPLER))
			{
				engineData->exec_glActiveTexture_GL_TEXTURE0(currentprogram->samplerState[action.udata1].registernumber);
				engineData->exec_glSetTexParameters(
					INT32_MAX
					,currentprogram->samplerState[action.udata1].d
					,((action.udata2 & 0xf0) >>4)
					,((action.udata2 & 0xf00) >>8)
					,(action.udata2 & 0xf));
			}
			break;
		...
		case RENDER_UPLOADVERTEXBUFFER:
		{
			...
			if (bufferID != UINT32_MAX)
			{
				...
				LOG(LOG_ERROR,"Context3D missing vertex bufferID, should not happen");
			break;
		}
...
void Context3D::setAttribs(EngineData *engineData,std::vector<RegisterMapEntry>& attributes)
{
	...
	while (it != attributes.end())
	{
		...
		else if (it->program_register_id != UINT32_MAX)
...
void Context3D::setSamplers(EngineData *engineData)
{
	for (uint32_t i = 0; i < currentprogram->samplerState.size();i++)
	{
		...
		{
			char buf[100];
			sprintf(buf,"sampler%d",currentprogram->samplerState[i].registernumber);
			sampid = engineData->exec_glGetUniformLocation(currentprogram->gpu_program,buf);
			if (sampid != UINT32_MAX)
			{
				engineData->exec_glUniform1i(sampid, currentprogram->samplerState[i].registernumber);
		...
		if (sampid != UINT32_MAX)
		{
			...
			LOG(LOG_ERROR,"sampler not found in program:"<<currentprogram->samplerState[i].registernumber<<" "<<currentprogram->gpu_program<<" "<<sampid<<" "<<currentprogram);
	}
}
...
Context3D::Context3D(ASWorker* wrk, Class_base *c)
	...
	,samplers{UINT32_MAX,UINT32_MAX,UINT32_MAX,UINT32_MAX,UINT32_MAX,UINT32_MAX,UINT32_MAX,UINT32_MAX,UINT32_MAX,UINT32_MAX,UINT32_MAX,UINT32_MAX,UINT32_MAX,UINT32_MAX,UINT32_MAX,UINT32_MAX}
...
void Context3D::sinit(lightspark::Class_base *c)
{
	...
	c->setDeclaredMethodByQName("profile","",c->getSystemState()->getBuiltinFunction(getProfile,0,Class<ASString>::getClassUninitialized(c->getSystemState())),GETTER_METHOD,true);
	c->setDeclaredMethodByQName("supportsVideoTexture","",c->getSystemState()->getBuiltinFunction(supportsVideoTexture,0,Class<Boolean>::getClassUninitialized(c->getSystemState())),GETTER_METHOD,false);
	...
	c->setDeclaredMethodByQName("createIndexBuffer","",c->getSystemState()->getBuiltinFunction(createIndexBuffer,1,Class<IndexBuffer3D>::getClassUninitialized(c->getSystemState())),NORMAL_METHOD,true);
	c->setDeclaredMethodByQName("createProgram","",c->getSystemState()->getBuiltinFunction(createProgram,0,Class<Program3D>::getClassUninitialized(c->getSystemState())),NORMAL_METHOD,true);
	c->setDeclaredMethodByQName("createCubeTexture","",c->getSystemState()->getBuiltinFunction(createCubeTexture,3,Class<CubeTexture>::getClassUninitialized(c->getSystemState())),NORMAL_METHOD,true);
	c->setDeclaredMethodByQName("createTexture","",c->getSystemState()->getBuiltinFunction(createTexture,4,Class<Texture>::getClassUninitialized(c->getSystemState())),NORMAL_METHOD,true);
	c->setDeclaredMethodByQName("createRectangleTexture","",c->getSystemState()->getBuiltinFunction(createRectangleTexture,4,Class<RectangleTexture>::getClassUninitialized(c->getSystemState())),NORMAL_METHOD,true);
	c->setDeclaredMethodByQName("createVertexBuffer","",c->getSystemState()->getBuiltinFunction(createVertexBuffer,2,Class<VertexBuffer3D>::getClassUninitialized(c->getSystemState())),NORMAL_METHOD,true);
	c->setDeclaredMethodByQName("createVideoTexture","",c->getSystemState()->getBuiltinFunction(createVideoTexture,0,Class<VideoTexture>::getClassUninitialized(c->getSystemState())),NORMAL_METHOD,true);
...
ASFUNCTIONBODY_ATOM(IndexBuffer3D,uploadFromByteArray)
{
	...
	memcpy(action.bufferdata.data(),data->getBufferNoCheck()+byteArrayOffset,count*sizeof(uint16_t));
...
ASFUNCTIONBODY_ATOM(IndexBuffer3D,uploadFromVector)
{
	...
	{
		asAtom a = data->at(i);
		uint16_t d = asAtomHandler::toUInt(a);
		action.bufferdata[i*2+1]=(d>>8)&0xff;
		action.bufferdata[i*2  ]=(d   )&0xff;
	}
	th->context->addAction(action);
}
...
ASFUNCTIONBODY_ATOM(VertexBuffer3D,uploadFromByteArray)
{
	...
	if (startVertex + numVertices > th->numVertices)
	{
		createError<RangeError>(wrk,kParamRangeError);
		return;
	}
	...
	memcpy(action.bufferdata.data(),data->getBufferNoCheck()+byteArrayOffset,numVertices*th->data32PerVertex*sizeof(float));
	th->context->addAction(action);
}
ASFUNCTIONBODY_ATOM(VertexBuffer3D,uploadFromVector)
{
	...
	if (data->size() < numVertices*th->data32PerVertex)
	{
		createError<RangeError>(wrk,kParamRangeError);
		return;
	}
