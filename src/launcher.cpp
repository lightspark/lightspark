...
extern void Decode85(const unsigned char* src, unsigned char* dst);
...
const char font_roboto_medium_compressed_data_base85[144680+1] =
...
const char* Launcher::getDefaultFontBase85()
{
	return font_roboto_medium_compressed_data_base85;
}
...
bool Launcher::start()
{
	...
	io.Fonts->AddFontFromMemoryCompressedBase85TTF(getDefaultFontBase85(), 24);
...
// returns the uncompressed ttf file of the default font, needed for nanoVG text rendering
// TODO move outside of launcher
unsigned char* getUncompressedDefaultFont(unsigned int& buf_decompressed_size)
{
	int compressed_ttf_size = (((int)strlen(font_roboto_medium_compressed_data_base85) + 4) / 5) * 4;
	unsigned char* compressed_ttf = new unsigned char[(size_t)compressed_ttf_size];
	Decode85((const unsigned char*)font_roboto_medium_compressed_data_base85, compressed_ttf);
	buf_decompressed_size = stb_decompress_length((const unsigned char*)compressed_ttf);
	unsigned char* buf_decompressed_data = new unsigned char[(size_t)buf_decompressed_size];
	stb_decompress(buf_decompressed_data, (const unsigned char*)compressed_ttf, (unsigned int)compressed_ttf_size);
	delete[] compressed_ttf;
