...
void ShapesBuilder::fillTokensFromSegemnt(vector<ShapePathSegment>& segments, tokenListRef* tokens)
{
	for (size_t j = 0; j < segments.size(); ++j)
	{
		...
		if (j == 0 || segment.from.key != segments[j-1].to.key) {
			tokens->tokens.push_back(GeomToken(MOVE).uval);
			tokens->tokens.push_back(GeomToken(segment.from).uval);
		}
		if (segment.quadctrl.key == segment.from.key || segment.quadctrl.key == segment.to.key) {
			tokens->tokens.push_back(GeomToken(STRAIGHT).uval);
			tokens->tokens.push_back(GeomToken(segment.to).uval);
		}
		else {
			tokens->tokens.push_back(GeomToken(CURVE_QUADRATIC).uval);
			tokens->tokens.push_back(GeomToken(segment.quadctrl).uval);
			tokens->tokens.push_back(GeomToken(segment.to).uval);
		}
	}
}
