...
void PulseStream::SoundManager::streamStatusCB(pa_stream* stream, SoundStream* th)
{
...
void PulseStream::SoundManager::fillAndSync(uint32_t id, uint32_t streamTime)
{
	assert(streams[id-1]);
	if(noServer==false)
	{
...
void PulseStream::SoundManager::streamWriteCB(pa_stream* stream, size_t frameSize, SoundStream* th)
{
	//Get buffer size
	int16_t* dest;
	pa_stream_begin_write(stream, (void**)&dest, &frameSize);
...
void PulseStream::SoundManager::freeStream(uint32_t id)
{
	pa_threaded_mainloop_lock(mainLoop);
...
uint32_t PulseStream::SoundManager::createStream(AudioDecoder* decoder)
{
	while(!contextReady);
	pa_threaded_mainloop_lock(mainLoop);
	uint32_t index=0;
...
void PulseStream::SoundManager::contextStatusCB(pa_context* context, SoundManager* th)
{
...
PulseStream::SoundManager::SoundManager():contextReady(false),noServer(false),stopped(false)
{
...
PulseStream::SoundManager::~SoundManager()
{
	stop();
}

void PulseStream::SoundManager::stop()
{
	if(!stopped)
	{
		stopped=true;
