...
enum BUILTIN_STRINGS { EMPTY=0, STRING_WILDCARD='*', ANY=BUILTIN_STRINGS_CHAR_MAX
					   ...
					   ,STRING_NUMBER
					   ,STRING_STRING
					   ,STRING_FUNCTION_LOWERCASE
					   ,STRING_ONROLLOVER
					   ,STRING_ONROLLOUT
					   ,STRING_PROTO
					   ,STRING_TARGET
					   ,STRING_FLASH_EVENTS_IEVENTDISPATCHER
					   ,STRING_ADDEVENTLISTENER
					   ,STRING_REMOVEEVENTLISTENER
					   ,STRING_DISPATCHEVENT
					   ,STRING_HASEVENTLISTENER
					   ,STRING_ONCONNECT
					   ,STRING_ONDATA
					   ,STRING_ONCLOSE
					   ,STRING_ONSELECT
...
enum CLASS_SUBTYPE { SUBTYPE_NOT_SET
					 ,SUBTYPE_PROXY
					 ,SUBTYPE_REGEXP
					 ,SUBTYPE_XML
					 ,SUBTYPE_XMLLIST
					 ,SUBTYPE_DATE
					 ,SUBTYPE_INHERIT
					 ,SUBTYPE_OBJECTCONSTRUCTOR
					 ,SUBTYPE_FUNCTIONOBJECT
					 ,SUBTYPE_GLOBAL
					 ,SUBTYPE_FUNCTION // 10
					 ,SUBTYPE_SYNTHETICFUNCTION
					 ,SUBTYPE_EVENT
					 ,SUBTYPE_WAITABLE_EVENT
					 ,SUBTYPE_INTERACTIVE_OBJECT
					 ,SUBTYPE_STAGE
					 ,SUBTYPE_LOADERINFO
					 ,SUBTYPE_CONTEXTMENU
					 ,SUBTYPE_BITMAP
					 ,SUBTYPE_BITMAPDATA
					 ,SUBTYPE_SOUND // 20
					 ,SUBTYPE_SOUNDTRANSFORM
					 ,SUBTYPE_SOUNDCHANNEL
					 ,SUBTYPE_PROGRESSEVENT
					 ,SUBTYPE_VECTOR
					 ,SUBTYPE_DISPLAYOBJECT
					 ,SUBTYPE_DISPLAYOBJECTCONTAINER
					 ,SUBTYPE_CONTEXTMENUBUILTINITEMS
					 ,SUBTYPE_SHAREDOBJECT
					 ,SUBTYPE_TEXTFIELD
					 ,SUBTYPE_TEXTFORMAT // 30
					 ,SUBTYPE_KEYBOARD_EVENT
					 ,SUBTYPE_MOUSE_EVENT
					 ,SUBTYPE_ROOTMOVIECLIP
					 ,SUBTYPE_MATRIX
					 ,SUBTYPE_POINT
					 ,SUBTYPE_RECTANGLE
					 ,SUBTYPE_COLORTRANSFORM
					 ,SUBTYPE_BYTEARRAY
					 ,SUBTYPE_APPLICATIONDOMAIN
					 ,SUBTYPE_LOADERCONTEXT // 40
					 ,SUBTYPE_SPRITE
					 ,SUBTYPE_MOVIECLIP
					 ,SUBTYPE_TEXTBLOCK
					 ,SUBTYPE_FONTDESCRIPTION
					 ,SUBTYPE_CONTENTELEMENT
					 ,SUBTYPE_ELEMENTFORMAT
					 ,SUBTYPE_TEXTELEMENT
					 ,SUBTYPE_ACTIVATIONOBJECT
					 ,SUBTYPE_TEXTLINE
					 ,SUBTYPE_STAGE3D // 50
					 ,SUBTYPE_MATRIX3D
					 ,SUBTYPE_INDEXBUFFER3D
					 ,SUBTYPE_PROGRAM3D
					 ,SUBTYPE_VERTEXBUFFER3D
					 ,SUBTYPE_CONTEXT3D
					 ,SUBTYPE_TEXTUREBASE
					 ,SUBTYPE_TEXTURE
					 ,SUBTYPE_CUBETEXTURE
					 ,SUBTYPE_RECTANGLETEXTURE
					 ,SUBTYPE_VIDEOTEXTURE // 60
					 ,SUBTYPE_VECTOR3D
					 ,SUBTYPE_NETSTREAM
					 ,SUBTYPE_WORKER
					 ,SUBTYPE_WORKERDOMAIN
					 ,SUBTYPE_MUTEX
					 ,SUBTYPE_AVM1FUNCTION
					 ,SUBTYPE_SAMPLEDATA_EVENT
					 ,SUBTYPE_BITMAPFILTER
					 ,SUBTYPE_GLOWFILTER
					 ,SUBTYPE_DROPSHADOWFILTER // 70
					 ,SUBTYPE_GRADIENTGLOWFILTER
					 ,SUBTYPE_BEVELFILTER
					 ,SUBTYPE_COLORMATRIXFILTER
					 ,SUBTYPE_BLURFILTER
					 ,SUBTYPE_CONVOLUTIONFILTER
					 ,SUBTYPE_DISPLACEMENTFILTER
					 ,SUBTYPE_GRADIENTBEVELFILTER
					 ,SUBTYPE_SHADERFILTER
					 ,SUBTYPE_THROTTLE_EVENT
					 ,SUBTYPE_CONTEXTMENUEVENT // 80
					 ,SUBTYPE_GAMEINPUTEVENT
					 ,SUBTYPE_GAMEINPUTDEVICE
					 ,SUBTYPE_VIDEO
					 ,SUBTYPE_MESSAGECHANNEL
					 ,SUBTYPE_CONDITION
					 ,SUBTYPE_FILE
					 ,SUBTYPE_FILEMODE
					 ,SUBTYPE_FILESTREAM
					 ,SUBTYPE_FILEREFERENCE
					 ,SUBTYPE_DATAGRAMSOCKET // 90
					 ,SUBTYPE_NATIVEWINDOW
					 ,SUBTYPE_EXTENSIONCONTEXT
					 ,SUBTYPE_SIMPLEBUTTON
					 ,SUBTYPE_SHAPE
					 ,SUBTYPE_MORPHSHAPE
					 ,SUBTYPE_URLLOADER
					 ,SUBTYPE_URLREQUEST
					 ,SUBTYPE_DICTIONARY
					 ,SUBTYPE_TEXTLINEMETRICS
					 ,SUBTYPE_XMLNODE // 100
					 ,SUBTYPE_XMLDOCUMENT
					 ,SUBTYPE_LOADER
					 ,SUBTYPE_TEXTJUSTIFIER
					 ,SUBTYPE_SPACEJUSTIFIER
					 ,SUBTYPE_EASTASIANJUSTIFIER
					 ,SUBTYPE_ERROR
					 ,SUBTYPE_SECURITYERROR
					 ,SUBTYPE_ARGUMENTERROR
					 ,SUBTYPE_DEFINITIONERROR
					 ,SUBTYPE_EVALERROR // 110
					 ,SUBTYPE_RANGEERROR
					 ,SUBTYPE_REFERENCEERROR
					 ,SUBTYPE_SYNTAXERROR
					 ,SUBTYPE_TYPEERROR
					 ,SUBTYPE_URIERROR
					 ,SUBTYPE_VERIFYERROR
					 ,SUBTYPE_UNINITIALIZEDERROR
					 ,SUBTYPE_AVM1SOUND
					 ,SUBTYPE_LOCALCONNECTION
					 ,SUBTYPE_NATIVEWINDOWBOUNDSEVENT // 120
					 ,SUBTYPE_AVM1MOVIECLIP
					 ,SUBTYPE_AVM1MOVIECLIPLOADER
					 ,SUBTYPE_GRAPHICSENDFILL
					 ,SUBTYPE_GRAPHICSSOLIDFILL
					 ,SUBTYPE_GRAPHICSPATH
					 ,SUBTYPE_AVM1MOVIE
					 ,SUBTYPE_URLVARIABLES
					 ,SUBTYPE_AVM1XMLDOCUMENT
					 ,SUBTYPE_AVM1XMLNODE
					 ,SUBTYPE_FOCUSEVENT // 130
					 ,SUBTYPE_STACKOVERFLOWERROR
					 ,SUBTYPE_AVM1SUPEROBJECT
					 ,SUBTYPE_FILEREFERENCELIST
					 ,SUBTYPE_HTTPSTATUSEVENT
					 ,SUBTYPE_CATCHSCOPEOBJECT
					 ,SUBTYPE_AVM1LOADVARS
					 ,SUBTYPE_STATUSEVENT
					 ,SUBTYPE_AVM1LOCALCONNECTION
					 ,SUBTYPE_TIMER
					 ,SUBTYPE_TIMEREVENT // 140
					 ,SUBTYPE_AVM1POINT
					 ...
					 ,SUBTYPE_AVM1TEXTFIELD
					 ,SUBTYPE_RESPONDER
				   };
 
enum STACK_TYPE{STACK_NONE=0,STACK_OBJECT,STACK_INT,STACK_UINT,STACK_NUMBER,STACK_BOOLEAN};

enum AS_BLENDMODE { BLENDMODE_NORMAL=0
					,BLENDMODE_LAYER=2
					,BLENDMODE_MULTIPLY=3
					,BLENDMODE_SCREEN=4
					,BLENDMODE_LIGHTEN=5
					,BLENDMODE_DARKEN=6
					,BLENDMODE_DIFFERENCE=7
					,BLENDMODE_ADD=8
					,BLENDMODE_SUBTRACT=9
					,BLENDMODE_INVERT=10
					,BLENDMODE_ALPHA=11
					,BLENDMODE_ERASE=12
					,BLENDMODE_OVERLAY=13
					,BLENDMODE_HARDLIGHT=14
					// blendmodes for internal use, not exposed to actionscript
					,BLENDMODE_INTERN_REPLACE=100 // overwrites the current pixels ignoring the alpha value (only used when rendering to bitmap)
				  };
...
enum HIT_TYPE { GENERIC_HIT, // point is over the object
				...
				GENERIC_HIT_INVISIBLE_INCLUDE_MASK, // include mask in hittesting
