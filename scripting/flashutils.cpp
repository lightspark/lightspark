...
void Timer::tick()
{
	_NR<TimerEvent> e(Class<TimerEvent>::getInstanceS("timer"));
	incRef();
	sys->currentVm->addEvent(_MR(this),e);
	if(repeatCount==0)
...
void IntervalRunner::tick() 
{
	...
	obj->incRef();
	_NR<FunctionEvent> event(new FunctionEvent(static_cast<IFunction*>(callback), obj, args, argslen));
