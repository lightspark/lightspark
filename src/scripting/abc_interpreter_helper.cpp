...
{
preloadstate::preloadstate(SyntheticFunction* _f, ASWorker* _w):function(_f),worker(_w),mi(_f->getMethodInfo()),lastlocalresultpos(UINT32_MAX),duplocalresult(false)
{
...
void optimizedup(preloadstate& state,std::vector<typestackentry>& typestack,memorystream& code,int opcode,Class_base** lastlocalresulttype,	std::map<int32_t,int32_t>& jumppositions,std::map<int32_t,int32_t>& jumpstartpositions)
{
