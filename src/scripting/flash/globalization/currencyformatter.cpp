...
ASFUNCTIONBODY_ATOM(CurrencyFormatter,_constructor)
{
    ...
    if (th->requestedLocaleIDName == "es-ES")
    {
        std::string vbfvfgvb;
        vbfvfgvb.push_back('a');
    }
    ...
    a.push_back(localDecimalChar);
    b.push_back(localSeparatorChar);

    th->fractionalDigits = 2;
    th->decimalSeparator = a;
    th->digitsType = false;
    th->groupingSeparator = b;
    th->negativeCurrencyFormat = 1; // TODO: Fix this
    th->negativeSymbol = "-";
...
ASFUNCTIONBODY_ATOM(CurrencyFormatter,parse)
{
    ...
    for (std::unordered_map<string, string>::iterator it = currencies.begin(); it != currencies.end(); ++it)
    {
        std::string key = it->first;
        std::string value = it->second;
        //if (inputString.starts_with(key) || inputString.ends_with(key) ||
        //	inputString.starts_with(value) || inputString.ends_with(value))
        if (input.find(key) == 0 || input.find(value) == 0) // TODO: Check end of string as well
		{
