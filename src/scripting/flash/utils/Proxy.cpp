...
multiname *Proxy::setVariableByMultiname(const multiname& name, asAtom& o, CONST_ALLOWED_FLAG allowConst)
{
	...
	if(proxySetter.isInvalid())
		return ASObject::setVariableByMultiname(name,o,allowConst);

	assert_and_throw(proxySetter.isFunction());
...
GET_VARIABLE_RESULT Proxy::getVariableByMultiname(asAtom& ret, const multiname& name, GET_VARIABLE_OPTION opt)
{
	...
	if (ret.isValid() || !implEnable || (opt & GET_VARIABLE_OPTION::SKIP_IMPL)!=0)
		return res;
...
bool Proxy::hasPropertyByMultiname(const multiname& name, bool considerDynamic, bool considerPrototype)
{
	...
	if(proxyHasProperty.isInvalid())
	{
		return false;
	}

	assert_and_throw(proxyHasProperty.isFunction());
...
bool Proxy::deleteVariableByMultiname(const multiname& name)
{
	...
	if(proxyDeleter.isInvalid())
	{
		return ASObject::deleteVariableByMultiname(name);
	}

	assert_and_throw(proxyDeleter.isFunction());
