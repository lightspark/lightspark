...
void InteractiveObject::defaultEventBehavior(Ref<Event> e)
{
	if(mouseEnabled && e->type == "contextMenu")
		getSystemState()->pushEvent(LSOpenContextMenuEvent(this));
}
...
bool DisplayObjectContainer::deleteVariableByMultiname(const multiname &name, ASWorker* wrk)
{
	...
	if (v && v->kind == TRAIT_KIND::DYNAMIC_TRAIT && v->isDisplayObjectVar())
	{
		DisplayObject* obj = v->getObjectVar()->as<DisplayObject>();
		if (!obj->legacy)
		{
			_removeChild(obj);
		}
