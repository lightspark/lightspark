...
void ASSocket::sinit(Class_base* c)
{
	...
	c->setDeclaredMethodByQName("connected","",c->getSystemState()->getBuiltinFunction(_connected,0,Class<Boolean>::getClassUninitialized(c->getSystemState())),GETTER_METHOD,true);
	c->setDeclaredMethodByQName("flush","",c->getSystemState()->getBuiltinFunction(_flush),NORMAL_METHOD,true);
	c->setDeclaredMethodByQName("bytesAvailable","",c->getSystemState()->getBuiltinFunction(bytesAvailable,0,Class<UInteger>::getClassUninitialized(c->getSystemState())),GETTER_METHOD,true);
