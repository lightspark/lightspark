...
enum EVENT_TYPE { EVENT=0
	...
	,TEXTINPUT_EVENT
	,IDLE_EVENT
	,SET_LOADER_CONTENT_EVENT
	,ROOTCONSTRUCTEDEVENT
	,LOCALCONNECTIONEVENT
	,GETMOUSETARGET_EVENT
	,RENDER_FRAME
	,BROADCAST_EVENT
	,STARTJOB_EVENT
	,GARBAGECOLLECTION_EVENT
	,FIRST_FRAME_AVAILABLE_EVENT
	,ADDTOINVALIDATEQUEUE_EVENT
};
...
friend class ABCVm;
private:
	tiny_string eventname;
public:
	BroadcastEvent(const tiny_string& _eventname);
	EVENT_TYPE getEventType() const override { return BROADCAST_EVENT; }
};
...
public:
	DisplayObject* dobj;
	AddToInvalidateQueueEvent(DisplayObject* _dobj):Event(nullptr,nullptr, "AddToInvalidateQueueEvent"),dobj(_dobj) {}
	EVENT_TYPE getEventType() const override { return ADDTOINVALIDATEQUEUE_EVENT; }
};
