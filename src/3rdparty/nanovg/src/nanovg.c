...
NVGcontext* nvgCreateInternal(NVGparams* params)
{
	...
	if (ctx->params.renderCreate && ctx->params.renderCreate(ctx->params.userPtr) == 0) goto error;
	...
	// Create font texture
	ctx->fontImages[0] = ctx->params.renderCreateTexture ? ctx->params.renderCreateTexture(ctx->params.userPtr, NVG_TEXTURE_ALPHA, fontParams.width, fontParams.height, 0, NULL) : 0;
	ctx->fontImageIdx = 0;
...
void nvg__deletePickScene(struct NVGpickScene* ps);
...
{
	ctx->params.renderUpdateTextureFlags(ctx->params.userPtr, image, imageFlags);
}
...
//	NVGstate* state = nvg__getState(ctx);
	...
	cache->bounds[0] = cache->bounds[1] = FLT_MAX;
	cache->bounds[2] = cache->bounds[3] = -FLT_MAX;
...
//-------------------- hittesting ---------------
// taken from https://github.com/memononen/nanovg/pull/230
NVGcontext* nvgCreatePick(int flags)
{
	NVGparams params;
	NVGcontext* ctx = NULL;
	memset(&params, 0, sizeof(params));
...
error:
	// 'pick' is freed by nvgDeleteInternal.
	if (ctx != NULL) nvgDeleteInternal(ctx);
	return NULL;
}
...
static NVGpickPath* nvg__pickPathCreate(NVGcontext* context, int id, int forStroke)
{
	...
	while (i < ncommands) {
		...
		switch (cmd) {
			...
			case NVG_CLOSE:
				if (ps->npoints &&
					(ps->points[(ps->npoints - 1) * 2] != start[0] ||
	...
	for (s = 0; s < nsegments; ++s, ++seg) {
		if ((seg->bounds[1] - NVG_PICK_EPS) < y &&
			...
			switch(seg->type) {
				...
				case NVG_BEZIERTO:
					if (seg->bounds[0] > x && seg->bounds[0]!= seg->bounds[2])
						// Line originates outside the box.
						++nintersections;
					else
						// Line originates inside the box.
